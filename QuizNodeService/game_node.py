...
class GameNode:
    ...
    def connect_to_room(self, user_socket: int, room_id: int):
        if room_id in self.rooms.keys():
            self.rooms[room_id].connect(self.sockets_users[user_socket])
        else:
            raise Exception(f'Connect to not existing room\n'
                            f'Socket: {0}, Player: {1}, Room: {2}',
                            user_socket, self.sockets_users[user_socket], room_id)

    def disconnect_from_room(self, user_socket: int):
        pass
